    ...
    app.bonus = {
        send_by_print_init: function () {
            ...
            var option = {
                ...
                messages: {
                    bonus_sum: {
                        required: bonus_js_lang.bonus_sum_required,
                        number: bonus_js_lang.bonus_number_required
                    },
                },
    ...
    app.link_goods = {
        ...
        load_link_goods_opt: function (data) {
            ...
            if (data.content.length > 0) {
                ...
                $('.nav-list-ready').html('<li class="ms-elem-selectable disabled"><span>' + bonus_js_lang.select_goods_empty + '</span></li>');
            }
            ...
            $('#ms-search').quicksearch(
                ...
                onAfter: function () {
                    $('.ms-group').each(function (index) {
                        $(this).find('.isShow').length ? $(this).css('display', 'block') : $(this).css('display',
                            'none');
                    });
                    return;
                },
        ...
        add_link_goods: function () {
            $('.nav-list-ready li')
                .on('click', function () {
                var $this = $(this),
                    tmpobj = $('<li class="ms-elem-selection"><input type="hidden" name="goods_id[]" value="' + $this.attr(
                        'data-id') + '" />' + $this.text() +
    ...
    app.link_user = {
        ...
        load_link_user_opt: function (data) {
            ...
            if (data.content != null && data.content.length > 0) {
                ...
                $('.nav-list-ready').html('<li class="ms-elem-selectable disabled"><span>' + bonus_js_lang.select_user_empty +
                    '</span></li>');
            }
        ...
        add_link_user: function () {
            $('.nav-list-ready li')
                .on('click', function () {
                var $this = $(this),
                    tmpobj = $('<li class="ms-elem-selection"><input type="hidden" name="user_id[]" value="' + $this.attr(
                        'data-id') + '" />' + $this.text() +
                        '<span class="edit-list"><i class="fontello-icon-minus-circled ecjiafc-red del"></i></span></li>');
                if (!$this.hasClass('disabled')) {
                    tmpobj.appendTo($(".ms-selection .nav-list-content"));
