...
class admin extends ecjia_admin {
	...
	public function add() 
	{
		...
		$this->assign('cfg_lang',    ecjia::config('lang'));
		...
		$this->assign_lang('bonus_arr ' , $bonus_arr );
	...
	public function insert() 
	{
		...
		// 		if (intval($_POST['type']) > 0 && !isset($_POST['type_ext'])) {
		// 			$this->showmessage('请选择类型', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		// 		}
...
// 		_dump($bonus_arr,1);
	...
	public function update() 
	{
		...
		$data = array(
				...
				//         	'usebonus_type_ext'	   => intval($_POST['bonus_type_ext']),
		);
	...
	public function edit_type_name() 
	{
		...
		/* 对编辑红包类型名称进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对编辑红包类型名称进行权限检查  BY：MaLiuWei  END */
	...
	public function edit_type_money() 
	{
		...
		/* 对编辑红包金额进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对编辑红包金额进行权限检查  BY：MaLiuWei  END */
		$id		= intval($_POST['pk']);
	...
	public function edit_min_amount() 
	{
		...
		/* 对编辑订单下限进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对编辑订单下限进行权限检查  BY：MaLiuWei  END */
		$id  = intval($_POST['pk']);
		$val = floatval($_POST['value']);
		/* 可为0 */
	...
	public function remove() 
	{
		...
		/* 对删除红包类型进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对删除红包类型进行权限检查  BY：MaLiuWei  END */
		$id = intval($_GET['id']);
		if(empty($_SESSION['ru_id'])){
	...
	public function send() 
	{
		...
		if ($send_by == SEND_BY_USER) {
			...
			//$other_goods_list = $this->db_goods->field('goods_id')->where('bonus_type_id > 0 AND bonus_type_id <> '.$id.'')->select();
			$other_goods_list = $this->db_goods->field('goods_id')->where($where_sql)->select();
...
// 	}
	...
	public function send_mail() 
	{
		...
		//$links[] = array('text' => RC_Lang::lang('back_bonus_list'), 'href' =>  RC_Uri::url('bonus/admin/bonus_list', 'bonus_type='. $bonus['bonus_type_id'])); ;
	...
	public function remove_bonus() 
	{
		...
		/* 对删除红包进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对删除红包进行权限检查  BY：MaLiuWei  END */
		$id = intval($_GET['id']);
